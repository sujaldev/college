...
\begin{document}
    ...
    \begin{frame}{What is RTC?}
        \begin{itemize}
            ...
            \item It stores seconds, minutes, hours, day, date, month, year in registers.
            ...
            \item Time is maintained using a 32.768 kHz crystal oscillator.
            \item Internal frequency division generates a 1 Hz signal (1 second tick).
            \item Time values are stored in registers in BCD (Binary Coded Decimal) format.
            \item RTC communicates with microcontrollers using protocols like I2C or SPI.
            \item Some RTCs provide additional features like alarms and square wave output.
        \end{itemize}
    \end{frame}
    ...
    \begin{frame}[t,allowframebreaks]{Working of DS1307 RTC}
        ...
        \begin{center}
            \includegraphics[width=0.8\textwidth]{ds1307-circuit}
        \end{center}
        ...
        \begin{center}
            \includegraphics[width=0.7\textwidth]{repeated-start}
        \end{center}
        ...
        \begin{enumerate}
            ...
            indicate a read operation. Register pointer will be auto incremented after transmitting each byte.
            \item Transmit a STOP bit condition.
        \end{enumerate}
    \end{frame}
    ...
    \begin{frame}[fragile,allowframebreaks]{Interfacing with AVR Microcontroller}
        ...
        \noindent%
        In the AVR microcontroller, five major registers are associated with the TWI (I2C is referred to as Two-Wire
        Interface). They are: \\
        \begin{enumerate}
            \item \textbf{TWI Bit Rate Register (TWBR):}
            ...
            \end{equation*}
            \item \textbf{TWI Status Register (TWSR):}
            The most-significant first five bits of TWSR are dedicated to show the status of the TWI logic and bus. The
            first two least-significant bits control the bit rate prescaler.
            \item \textbf{TWI Control Register (TWCR):} TWCR controls the operation of the TWI.
            \begin{enumerate}
                \item \textbf{Bit 0 -- TWIE (TWI Interrupt Enable):}
                Making this bit HIGH enables the TWI interrupt if the general interrupt is enabled.
                \item \textbf{Bit 2 -- TWEN (TWI Enable):} Making this bit HIGH enables the TWI module.
                \item \textbf{Bit 3 -- TWWC (TWI Write Collison Flag):}
                This bit is set HIGH when we attempt to access the TWI Data Register when TWINT is low. This flag is
                cleared by writing to the TWDR register when TWINT is high.
                \item \textbf{Bit 4 -- TWSTO (TWI STOP Condition bit):}
                In master mode, making this bit HIGH causes the TWI to generate a STOP condition. This bit is cleared by
                hardware when the STOP condition is transmitted.
                \item \textbf{Bit 5 -- TWSTA (TWI START Condition bit):}
                Making this bit HIGH will generate a START condition if the bus is free; otherwise, the TWI module waits
                for the bus to become free and then generates a START condition.
                \item \textbf{Bit 6 -- TWEA (TWI Enable Acknowledge):}
                Making this bit HIGH will enable the generation of ACK when needed in slave or receiver mode.
                \item \textbf{Bit 7 -- TWINT (TWI Interrupt):}
                This bit is set by hardware when the TWI module has finished its current job. If the TWI and general
                interrupt are enabled, changing TWINT to one will cause the MCU to jump to the TWI interrupt vector.
                Clearing this flag starts the operation of the TWI. TWINT must be cleared by software.
            \end{enumerate}
            \item \textbf{TWI Data Register (TWDR):}
            In Receive mode, the last received byte will be in the TWDR, and in Transmit mode, you should write the next
            byte into TWDR to be transmitted. We can access the TWDR only when the TWIE is set to one otherwise
            collision happens.
            \item \textbf{TWI Address Register (TWAR):}
...
A1: DEC     R22                  ;transmit STOP condition
    ...
    RET
        ...
        \textbf{\large Setting the date in assembly:}
        \\~\\
        \begin{minted}[fontsize=\scriptsize,ignorelexererrors=true]{asm}
.INCLUDE "M32DEF.INC"
...
DELAY:
    LDI   R22, 0xFF
A1:
    DEC   R22              ; transmit STOP condition
    NOP
    BRNE  A1
    RET
        \end{minted}
        ...
        \textbf{\large Setting the time in C:}
        \\~\\
        \begin{minted}[fontsize=\scriptsize,ignorelexererrors=true]{C}
...
int main(void)
{
    ...
    return 0;
        ...
        \textbf{\large Setting the date in C:}
        \\~\\
        \begin{minted}[fontsize=\scriptsize,ignorelexererrors=true]{C}
#include <avr/io.h>              //standard AVR header
...
int main(void)
{
    ...
    \begin{frame}{Applications of RTC}
        \begin{itemize}
            \item Digital clocks and watches
            \item Data logging systems
            \item Embedded systems requiring timestamps
            \item Alarm systems
            \item IoT devices
        \end{itemize}
    \end{frame}
    ...
    \begin{frame}{Example System}
        \begin{itemize}
            \item RTC + Microcontroller + LCD
            \item RTC stores current time
            \item MCU reads time using I2C
            \item Time displayed on LCD
        \end{itemize}
    \end{frame}
